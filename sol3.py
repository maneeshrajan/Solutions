...
def Distance_str(str1, str2, m , n): 
    ...
    if str1[m-1]==str2[n-1]: 
        return Distance_str(str1,str2,m-1,n-1) 
  
    # If last characters are not same, consider all three 
    # operations on last character of first string, recursively 
    # compute minimum cost for all three operations and take 
    # minimum of three values. 
    return 1 + min(Distance_str(str1, str2, m, n-1), Distance_str(str1, str2, m-1, n),Distance_str(str1, str2, m-1, n-1))
...
def set_priority(search_results, partial):
    ...
    for item in search_results:
        priority.append([item,item.find(partial),word_count[item],len(item)]) #creating a list of lists with words containing the partial word and their frequency, starting index and length 
...
if l ==[]:	#This indicates that the word given as input is not present in any of the words in list of words given
    ...
    l=set_priority(word_search((sys.argv)[2]),sys.argv[2])
print(*l,sep=', ')
